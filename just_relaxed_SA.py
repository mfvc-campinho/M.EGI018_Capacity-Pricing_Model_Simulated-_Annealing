...
--------------------------------
1. Optimization Phase (FAST):
   - SA runs using the RELAXED LP Model (Continuous Capacity).
   - Solves ~2x faster than MIP.
   
2. Final Phase (ACCURATE):
   - Takes the Best Pricing Policy found by SA.
   - Runs the INTEGER MIP Model ONCE to calculate exact fleet and profit.
   - Exports the valid integer solution.
"""
# ==============================================================================
import os
import sys
...
class PersistentEvaluator:
    def __init__(self, data, is_relaxed=False):
        # If is_relaxed=True, variables are Continuous (Fast LP).
        # If is_relaxed=False, variables are Integer (Exact MIP).
        ...
        # Flow Variables (Always Relaxed for speed)
    ...
    def solve_for_pricing(self, pricing_policy):
        ...
        if res.solver.termination_condition == TerminationCondition.optimal:
            ...
            # Extract capacity if needed (only for final save)
            current_owned = {}
...
def perturb_pricing(pricing_in, P_max, mutation_rate=0.02):
    ...
    for key in targets:
        new_p = random.randint(0, P_max - 1)
        pricing[key] = new_p
    return pricing
...
def run_sa_fast_relaxation(instance, max_seconds=120):
    print(f"Running SA (Fast Relaxation) for {instance}")
    
    data = InstanceData(instance)
    
    # 1. Setup Evaluators
    print("\n--- INITIALIZING EVALUATORS ---")
    # A. LP Evaluator (Fast, Relaxed) -> Used for SA Loop
    evaluator_lp = PersistentEvaluator(data, is_relaxed=True)
    
    # B. MIP Evaluator (Integer, Exact) -> Used ONLY at the end
    evaluator_mip = PersistentEvaluator(data, is_relaxed=False)
    
    # 2. Initial Solution (Greedy Prices)
    curr_price = generate_greedy_pricing(data)
    
    print("Evaluating Initial Solution (Relaxed)...")
    curr_obj, _ = evaluator_lp.solve_for_pricing(curr_price)
    print(f"Initial Relaxed Profit: {curr_obj:,.0f}")
    
    best_obj_lp = curr_obj
    best_price = copy.deepcopy(curr_price)
    
    # SA Params
    T = 5000; alpha = 0.90; iters_per_T = 1
    start_time = time.time()
    total_iter = 0
    
    print("\nIter  | Temp       | New LP Obj      | Best LP Obj     | Eval(s)  | Status")
    print("-" * 90)
    
    while (time.time() - start_time) < max_seconds:
        total_iter += 1
        
        # 2. Perturb Pricing (Neighbor)
        n_price = perturb_pricing(curr_price, data.P, mutation_rate=0.02)
        ...
        print(f"{total_iter:<5} | {T:<10.2f} | {n_obj:<15,.0f} | {best_obj_lp:<15,.0f} | {t1-t0:<8.4f} | {status}")
        
        T *= alpha
    
    print(f"\nSA Loop Finished. Best Relaxed Profit: {best_obj_lp:,.0f}")
    
    # 5. FINAL STEP: VALIDATE WITH INTEGER MIP
    print("\n--- FINAL VALIDATION (MIP) ---")
    print("Running exact Integer optimization on best found pricing policy...")
    ...
    print(f"Validation Time: {end_mip - start_mip:.2f}s")
    
    save_result("BestSolution_Inst41_PricingSA.xlsx", final_fleet, best_price)

if __name__ == "__main__":
    INSTANCE = r"data\Inst42.xlsx"
