...
*** MODIFIED TO SOLVE WITH FIXED CAPACITY (w_O and w_L) ***

This script fixes the fleet acquisition variables (w_O, w_L) to
pre-defined values (all 0 for this run) and solves the
remaining MILP for pricing and allocation.
"""
...
def solve_instance(filename):
    ...
    try:
        ...
        # ONY and ONU parameters (from paper, defaulting to 0)
        ...
        # ==================================================================
        # ========== NEW: FIXING CAPACITY VARIABLES (USER REQUEST) ========
        # ==================================================================
        print("\n*** MODIFICATION: Fixing capacity variables to 1 as requested. ***")
        
        # 1. Define the fixed values (all zeros for this run)
        w_O_fixed = {(g, s): 1 for g in model.G for s in model.S}
        w_L_fixed = {(g, t, s): 1 for g in model.G for t in model.T_minus for s in model.S}

        # 2. Fix the variables in the model
        # The .fix() method locks the variable to the specified value.
        ...
        print("*** Capacity variables w_O and w_L fixed to 0. ***\n")
        # ==================================================================
        ...
        model.obj = Objective(sense=maximize, rule=lambda m:
                              ...
                              # Buying Costs (will be 0 since w_O is fixed)
        ...
        def stock_O(m, g, t, s):
            if t == 0:
                # Eq. (5) from paper
                # w_O is fixed, so x_O[g,0,s] will be fixed to INX_gs[g,s]
        ...
        def stock_L(m, g, t, s):
            ...
            # w_L is fixed, so 'acq' and 'ret' will always be 0
        ...
        # This constraint is now trivial (0 <= BUD) but harmless to leave
...
# BATCH LOOP
# ==============================================================================
RESULTS_FILE = "Fixed_Capacity_Results.xlsx" # Changed output file name
results = []

print("Starting batch run (Fixed Capacity, Instance 1 only)...")
