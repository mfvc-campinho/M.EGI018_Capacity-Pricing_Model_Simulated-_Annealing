...
CAR RENTAL FLEET MANAGEMENT - PARTIALLY RELAXED MILP SOLVER
*** MODIFIED to import fleet capacity from the heuristic file ***

This script:
1. Imports a capacity solution (w_O, w_L) from heuristic.py
2. Fixes the w_O and w_L variables to those values.
3. Keeps pricing 'q' as Binary.
4. Relaxes all other integer variables (x, y, u, f) to continuous.
5. Solves the resulting MILP for pricing and allocation.
"""
...
import pandas as pd
import numpy as np
from constructive_heuristic import ConstructiveHeuristic # <-- IMPORTING THE NEW CLASS
...
def solve_instance(filename):
    print(f"\n{'='*60}")
    print(f"PROCESSING PARTIALLY-RELAXED MILP (FIXED HEURISTIC): {filename}")
    print(f"{'='*60}")
    ...
    try:
        ...
        print("\n*** RUNNING AS A PARTIAL MILP (q=Binary, others=Relaxed) ***\n")
        ...
        # ----- Decision Variables (PARTIALLY RELAXED) -----
        model.w_O = Var(model.G, model.S, domain=NonNegativeReals) # RELAXED
        model.w_L = Var(model.G, model.T_minus, model.S, domain=NonNegativeReals) # RELAXED
        
        # --- q is KEPT AS BINARY ---
        model.q = Var(model.R, model.A, model.P, domain=Binary) # KEPT AS BINARY
        
        model.x_L = Var(model.G, model.T, model.S, domain=NonNegativeReals) # RELAXED
        model.x_O = Var(model.G, model.T, model.S, domain=NonNegativeReals) # RELAXED
        model.y_L = Var(model.S, model.S, model.G, model.T_minus, domain=NonNegativeReals) # RELAXED
        model.y_O = Var(model.S, model.S, model.G, model.T_minus, domain=NonNegativeReals) # RELAXED
        model.u_L = Var(model.R, model.A, model.G, domain=NonNegativeReals) # RELAXED
        model.u_O = Var(model.R, model.A, model.G, domain=NonNegativeReals) # RELAXED
        model.f_L = Var(model.G, model.T, domain=NonNegativeReals) # RELAXED
        model.f_O = Var(model.G, model.T, domain=NonNegativeReals) # RELAXED
        model.U = Var(model.R, model.A, domain=NonNegativeReals) # RELAXED
        model.v = Var(model.R, model.A, model.P, domain=NonNegativeReals) # RELAXED

        # ==================================================================
        # ========== NEW: FIXING CAPACITY FROM HEURISTIC ========
        # ==================================================================
        print("\n*** MODIFICATION: Generating and fixing capacity from external heuristic... ***")
        
        # 1. Initialize the heuristic class
        heuristic = ConstructiveHeuristic()
        
        # 2. Generate the fixed solution dictionaries
        #    (You can swap this for other methods, e.g., generate_zero_solution)
        ...
        # To run with ZERO fleet, uncomment the line below:
        # w_O_fixed, w_L_fixed = heuristic.generate_zero_solution(model.G, model.S, model.T_minus)

        # 3. Fix the variables in the model
        ...
        # =================SOLVE=================
        print(f"\n[Gurobi Output for {filename} (Partial MILP, Fixed Fleet) follows...]")
        ...
        print("\n--- Sample Partial MILP Solution Results ---")
        ...
        print("\nPricing (q[r,a,p]) - (Should be 0 or 1):")
        count_frac = 0
        for r in model.R:
            for a in model.A:
                for p in model.P:
        ...
        if count_frac > 0:
            ...
            print("  All q values are integer (0 or 1), as expected.")
        print("---------------------------------")
        ...
        return {'Instance': filename, 'Status': 'Error', 'Objective': str(e), 'Gap': 'N_A'}


# ==============================================================================
# BATCH LOOP
# ==============================================================================
RESULTS_FILE = "Partial_Relaxation_Fixed_Heuristic_Results.xlsx" # Changed output file name
results = []

print("Starting batch run (Partial Relaxation, Fixed Heuristic, Instance 1 only)...")

# This loop runs ONLY for i=1
for i in range(1, 2):
    fname = f"data\Inst{40}.xlsx"
