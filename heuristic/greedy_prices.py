...
CAR RENTAL FLEET MANAGEMENT - FULLY FIXED SIMULATION
*** FIXED CAPACITY: 1 unit per group/station ***
*** FIXED PRICE: "Greedy" (Pre-selected for max potential revenue) ***

This is effectively a SIMULATION. It solves instantly.
It answers: "If I have 1 car everywhere and always pick the highest revenue price,
how do I manage the logistics (transfers/upgrades)?"
"""
...
def solve_instance(filename):
    print(f"\n{'='*60}")
    print(f"PROCESSING FIXED-FIXED SIMULATION: {filename}")
    print(f"{'='*60}")
    ...
    try:
        ...
        # =======================================
        print("Pre-calculating Greedy Prices...")
        # For every (r, a), pick the 'p' that maximizes (Price * Demand)
        # This removes the decision variable 'q' entirely.
        
        target_p = {} # Stores the chosen p for each (r,a)
        ...
        for r in range(R):
            for a in range(A+1):
                ...
                best_p = 0
                for p in range(P):
                    # Potential Revenue = Price * Total Demand at that price
                    # (We use DEM_rap, assuming we could fill it all)
        ...
        print("  Prices fixed to 'Best Potential Revenue' settings.")
        ...
        # v is also simplified, but we can keep it for structure
        ...
        # ----- Objective Function -----
        # Note: We don't need 'q' in the objective, because 'v' handles the revenue logic
        ...
        # Linearization Replacement:
        # Since Price is fixed, v[r,a,p] is simply the sales at that price.
        # It is 0 if p != target_p[r,a].
        # It is <= Demand if p == target_p[r,a].
        
        def v_fixed_rule(m, r, a, p):
            if p == target_p[(r, a)]:
                # If this is the chosen price, sales <= demand
                return m.v[r, a, p] <= DEM_rap[r, a, p]
            else:
                # If this is NOT the chosen price, sales = 0
                return m.v[r, a, p] == 0
        model.c_v_fixed = Constraint(model.R, model.A, model.P, rule=v_fixed_rule)

        # Total fulfillment U must equal sum of v (since only one v is active)
        model.c_U_link = Constraint(model.R, model.A, rule=lambda m, r, a: m.U[r, a] == sum(m.v[r, a, p] for p in m.P))
        
        # Physical Fulfillment link
        model.c1 = Constraint(model.R, model.A, rule=lambda m, r, a: m.U[r, a] == sum(m.u_L[r, a, g]+m.u_O[r, a, g] for g in m.G))
        
        # Fleet Flow Constraints (Same as always)
        def stock_O(m, g, t, s):
            if t == 0:
        ...
        # =================SOLVE=================
        print(f"\n[Gurobi Output for {filename} (Fully Fixed) follows...]")
...
# BATCH LOOP
# ==============================================================================
RESULTS_FILE = "Fully_Fixed_Simulation_Results.xlsx"
results = []

print("Starting batch run (Fully Fixed, Instance 1 only)...")
