...
CAR RENTAL FLEET MANAGEMENT - CAPACITY PLANNING MODEL
*** PRICE: Fixed (Greedy pre-calculation) ***
*** CAPACITY: Optimized (Decision Variable) ***

This model answers:
"If we strictly follow a 'Highest Potential Revenue' pricing strategy,
what is the optimal fleet size and mix we should acquire?"
"""
...
def solve_instance(filename):
    print(f"\n{'='*60}")
    print(f"PROCESSING CAPACITY OPTIMIZATION (FIXED PRICE): {filename}")
    print(f"{'='*60}")
    ...
    try:
        ...
        # =======================================
        print("Pre-calculating Greedy Prices (Max Revenue Potential)...")
        target_p = {}
        ...
        for r in range(R):
            for a in range(A+1):
                ...
                best_p = 0
                for p in range(P):
                    # Maximize Price * Demand
                    current_rev = PRI_pg[p, rental_gr[r]] * DEM_rap[r, a, p]
                    if current_rev > best_rev:
                        best_rev = current_rev
                        best_p = p
                target_p[(r, a)] = best_p
                
        print("  Prices fixed. Optimizing capacity for this strategy...")
        ...
        # ----- Decision Variables -----
        # CAPACITY IS FREE (Optimized)
        # Using NonNegativeIntegers to get a realistic fleet plan.
        # If still too slow, change to NonNegativeReals.
        ...
        model.obj = Objective(sense=maximize, rule=lambda m:
                              # Revenue (Fixed Price Strategy)
                              sum(m.v[r, a, p] * PRI_pg[p, rental_gr[r]] for r in m.R for a in m.A for p in m.P) -
                              # Costs (Solver tries to minimize these while maximizing revenue)
        ...
        # 1. Fixed Price Constraint
        # Restrict 'v' (sales) to only happen at the pre-calculated 'best_p'
        def v_fixed_rule(m, r, a, p):
            if p == target_p[(r, a)]:
                # Allowed to sell up to demand limit at this price
                return m.v[r, a, p] <= DEM_rap[r, a, p]
            else:
                # Forced to 0 at other prices
                return m.v[r, a, p] == 0
        model.c_v_fixed = Constraint(model.R, model.A, model.P, rule=v_fixed_rule)

        # 2. Link Total Fulfillment U to Sales v
        model.c_U_link = Constraint(model.R, model.A, rule=lambda m, r, a: m.U[r, a] == sum(m.v[r, a, p] for p in m.P))
        
        # 3. Link Physical cars (u) to Total Fulfillment U
        model.c1 = Constraint(model.R, model.A, rule=lambda m, r, a: m.U[r, a] == sum(m.u_L[r, a, g]+m.u_O[r, a, g] for g in m.G))
        
        # 4. Fleet Flow Constraints (Standard)
        def stock_O(m, g, t, s):
            if t == 0:
        ...
        try:
            ...
            gap_str = "N/A"
            if stat == 'optimal': gap_str = "0.00%"
            elif stat == 'maxTimeLimit': gap_str = "Unknown (Time Limit)"
        ...
        if obj_val != "N/A":
            print("\n--- OPTIMIZED CAPACITY RESULTS ---")
            
            total_owned = sum(value(model.w_O[g,s]) for g in model.G for s in model.S)
            print(f"Total Owned Cars Bought: {total_owned:.0f}")
            if total_owned > 0:
                for g in model.G:
                    for s in model.S:
                        if value(model.w_O[g,s]) > 0:
                            print(f"  Group {g}, Station {s}: {value(model.w_O[g,s]):.0f}")

            total_leased = sum(value(model.w_L[g,t,s]) for g in model.G for t in model.T_minus for s in model.S)
            print(f"\nTotal Leased Cars Acquired: {total_leased:.0f}")
            
            print(f"\nBudget Used: {sum(value(model.w_O[g, s])*COS_g[g] for g in model.G for s in model.S):.2f} / {BUD:.2f}")
...
# BATCH LOOP
# ==============================================================================
RESULTS_FILE = "FixedPrice_OptimizedCapacity_Results.xlsx"
results = []

print("Starting batch run (Fixed Price, Optimized Capacity, Instance 1 only)...")
