...
MILP SOLVER with WARM START (Heuristic Injection)
-------------------------------------------------
- Instance: Inst41
- Warm Start: Loads 'constructive_solutions/Solution_Inst41.xlsx'
- Solver: Gurobi (Direct)
"""
# ==============================================================================
import os
import sys
...
# 1. WARM START LOADER
# ==============================================================================
def apply_warm_start(model, heuristic_path):
    """
    Reads the heuristic solution and sets variable values as a MIP Start.
    """
    print(f"\n[Warm Start] Loading from: {heuristic_path}")
    if not os.path.exists(heuristic_path):
        print("  Warning: Heuristic file not found. Starting cold.")
        return

    try:
        xls = pd.ExcelFile(heuristic_path)
        
        # 1. OWNED CAPACITY
        # Expected Cols: Group, Station, Quantity
        if 'Owned_Capacity' in xls.sheet_names:
            df_o = pd.read_excel(xls, 'Owned_Capacity')
            count_o = 0
            for _, row in df_o.iterrows():
                g = int(row['Group']) - 1
                s = int(row['Station']) - 1
                val = float(row['Quantity'])
                if (g, s) in model.w_O:
                    model.w_O[g, s].value = val
                    count_o += 1
            print(f"  -> Set {count_o} owned variables.")

        # 2. LEASED CAPACITY
        # Expected Cols: Group, Time, Station, Quantity
        if 'Leased_Capacity' in xls.sheet_names:
            df_l = pd.read_excel(xls, 'Leased_Capacity')
            count_l = 0
            for _, row in df_l.iterrows():
                ...
                s = int(row['Station']) - 1
                val = float(row['Quantity'])
                # Ensure index exists (Time T vs T-1 check)
                if (g, t, s) in model.w_L:
                    model.w_L[g, t, s].value = val
                    count_l += 1
            print(f"  -> Set {count_l} leased variables.")

        # 3. PRICING
        # Expected Cols: RentalID, Antecedence, PriceLevel
        if 'Pricing_Policy' in xls.sheet_names:
            df_p = pd.read_excel(xls, 'Pricing_Policy')
            count_p = 0
            for _, row in df_p.iterrows():
                r = int(row['RentalID'])
                a = int(row['Antecedence'])
                p_target = int(row['PriceLevel'])
                
                # Set the chosen price to 1, others to 0
                for p in model.P:
                    if p == p_target:
                        model.q[r, a, p].value = 1
                    else:
                        model.q[r, a, p].value = 0
                count_p += 1
            print(f"  -> Set {count_p} pricing decisions.")
            
    except Exception as e:
        print(f"  Error loading warm start: {e}")
        print("  Continuing without full warm start...")
...
def solve_instance(filename, heuristic_file, time_limit=120):
    ...
    try:
        ...
        # APPLY WARM START
        # ======================================================================
        apply_warm_start(model, heuristic_file)
        ...
        if status in ['optimal', 'maxTimeLimit', 'feasible']:
            ...
            with pd.ExcelWriter("MILP_BestSolution_Inst41.xlsx") as writer:
                pd.DataFrame(data_o).to_excel(writer, sheet_name='Owned_Capacity', index=False)
                pd.DataFrame(data_l).to_excel(writer, sheet_name='Leased_Capacity', index=False)
                pd.DataFrame(data_p).to_excel(writer, sheet_name='Pricing_Policy', index=False)
            print("Done.")
...
if __name__ == "__main__":
    ...
    HEURISTIC_FILE = r"constructive_solutions/Solution_Inst41.xlsx"
    
    solve_instance(INSTANCE_FILE, HEURISTIC_FILE, time_limit=120)