...
CAR RENTAL FLEET MANAGEMENT - PURE LP (RELAXED) SOLVER
*** MODIFIED to run as an LP with FIXED capacity from heuristic ***
...
4. Solves the resulting LP for pricing and allocation.
"""
...
def solve_instance(filename):
    print(f"\n{'='*60}")
    print(f"PROCESSING LP RELAXATION (FIXED HEURISTIC): {filename}")
    print(f"{'='*60}")
    ...
    try:
        ...
        model.q = Var(model.R, model.A, model.P, domain=NonNegativeReals, bounds=(0, 1)) # RELAXED
        ...
        # =================SOLVE=================
        print(f"\n[Gurobi Output for {filename} (LP, Fixed Fleet) follows...]")
...
# BATCH LOOP
# ==============================================================================
RESULTS_FILE = "LP_Relaxation_Fixed_Heuristic_Results.xlsx" # Changed output file name
results = []

print("Starting batch run (LP Relaxation, Fixed Heuristic, Instance 1 only)...")
